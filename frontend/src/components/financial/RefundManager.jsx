            ...
            >
              ...
              {transactions.map((tx) => (
                <option key={`${tx._id}:${tx.type}`} value={`${tx._id}:${tx.type}`}>
                  {tx.type.toUpperCase()} - GH₵{tx.amount.toFixed(2)} ({new Date(tx.created_at).toLocaleDateString()})
                </option>
              ))}
            </select>
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="theme-heading font-semibold">
                      GH₵{refund.refund_amount.toFixed(2)} - {refund.transaction_type.toUpperCase()}
                    </p>
